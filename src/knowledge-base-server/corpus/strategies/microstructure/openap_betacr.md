...
entry_type: strategy
id: openap_betacr
canonical_name: Illiquidity-market return beta (beta4i)
aliases:
- Illiquidity-market return beta (beta4i)
- betaCR
one_line: Cross-sectional equity anomaly that uses Illiquidity-market return beta
  (beta4i) to long high-signal stocks and short low-signal stocks.
...
seminal_papers:
- title: Illiquidity-market return beta (beta4i)
  authors:
  - Acharya
  - Pedersen
  year: 2005
...
---
## Thesis
Illiquidity-market return beta (beta4i) is represented in the OpenAP signal catalog as a liquidity predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute betaCR for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=betaCR; category=liquidity; data=Trading; evidence=in-sample only. Review the generated entry before using it as a final public corpus item.
