...
entry_type: strategy
id: openap_betacc
canonical_name: Illiquidity-illiquidity beta (beta2i)
aliases:
- Illiquidity-illiquidity beta (beta2i)
- betaCC
one_line: Cross-sectional equity anomaly that uses Illiquidity-illiquidity beta (beta2i)
...
seminal_papers:
- title: Illiquidity-illiquidity beta (beta2i)
  authors:
  - Acharya
  - Pedersen
  year: 2005
...
---
## Thesis
Illiquidity-illiquidity beta (beta2i) is represented in the OpenAP signal catalog as a liquidity predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute betaCC for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=betaCC; category=liquidity; data=Trading; evidence=in-sample only. Review the generated entry before using it as a final public corpus item.
