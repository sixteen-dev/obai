...
entry_type: strategy
id: openap_betarc
canonical_name: Return-market illiquidity beta
aliases:
- Return-market illiquidity beta
- betaRC
one_line: Cross-sectional equity anomaly that uses Return-market illiquidity beta
...
seminal_papers:
- title: Return-market illiquidity beta
  authors:
  - Acharya
  - Pedersen
  year: 2005
...
---
## Thesis
Return-market illiquidity beta is represented in the OpenAP signal catalog as a liquidity predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute betaRC for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=betaRC; category=liquidity; data=Trading; evidence=in-sample only. Review the generated entry before using it as a final public corpus item.
