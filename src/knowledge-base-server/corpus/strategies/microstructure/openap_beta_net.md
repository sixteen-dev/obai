...
entry_type: strategy
id: openap_beta_net
canonical_name: Net liquidity beta (betanet,p)
aliases:
- Net liquidity beta (betanet,p)
- betaNet
one_line: Cross-sectional equity anomaly that uses Net liquidity beta (betanet,p)
...
seminal_papers:
- title: Net liquidity beta (betanet,p)
  authors:
  - Acharya
  - Pedersen
  year: 2005
...
---
## Thesis
Net liquidity beta (betanet,p) is represented in the OpenAP signal catalog as a liquidity predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute betaNet for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=betaNet; category=liquidity; data=Trading; evidence=in-sample only. Review the generated entry before using it as a final public corpus item.
