---
entry_type: strategy
id: openap_return_skew3f
canonical_name: Idiosyncratic skewness (3F model)
aliases:
- Idiosyncratic skewness (3F model)
- RetSkew3F
- ReturnSkew3F
one_line: Cross-sectional equity anomaly that uses Idiosyncratic skewness (3F model)
...
- 'Original-paper replication evidence: t=4.4 in port sort; reported long-short return=0.45,
  t-stat=4.35.'
...
seminal_papers:
- title: Idiosyncratic skewness (3F model)
  authors:
  - Bali, Engle
  - Murray
  year: 2015
...
---
## Thesis
Idiosyncratic skewness (3F model) is represented in the OpenAP signal catalog as a risk predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute ReturnSkew3F for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=ReturnSkew3F; category=risk; data=Price; evidence=t=4.4 in port sort. Review the generated entry before using it as a final public corpus item.
