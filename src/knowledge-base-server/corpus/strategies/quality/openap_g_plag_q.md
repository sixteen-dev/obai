...
entry_type: strategy
id: openap_g_plag_q
canonical_name: Gross Profits / Total Assets (Lagged Quarterly)
aliases:
- GPlag_q
...
## Signal intuition
The signal definition is: Revenue (revtq) - cost of goods solds (cogsq), divided by one quarter lagged total assets (atq). The source direction is to rank stocks by the signal and form the source-defined long-short spread.

## Construction sketch
```text
for each rebalance date:
    compute GPlag_q for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=GPlag_q; category=profitability; data=Accounting; evidence=HXZ variant. Review the generated entry before using it as a final public corpus item.
