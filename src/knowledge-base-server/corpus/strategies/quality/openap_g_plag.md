...
entry_type: strategy
id: openap_g_plag
canonical_name: Gross Profits / Total Assets (Lagged)
aliases:
- GPlag
...
## Signal intuition
The signal definition is: Revenue (sale) - cost of goods solds (cogs), divided by 12 months lagged total assets. The source direction is to rank stocks by the signal and form the source-defined long-short spread.

## Construction sketch
```text
for each rebalance date:
    compute GPlag for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=GPlag; category=profitability; data=Accounting; evidence=HXZ variant. Review the generated entry before using it as a final public corpus item.
