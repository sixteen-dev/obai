---
entry_type: strategy
id: openap_oper_prof_lag
canonical_name: Operating Profits / Book Equity (Lagged)
aliases:
- OperProfLag
- ProfOperLag
...
## Signal intuition
The signal definition is: Revenue (revt) minus cost (cogs) - administrative expenses (xsga) - interest expenses (xint), scaled by lagged book value of equity (ceq). Exclude smallest size tercile. The source direction is to rank stocks by the signal and form the source-defined long-short spread.

## Construction sketch
```text
for each rebalance date:
    compute OperProfLag for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=OperProfLag; category=profitability; data=Accounting; evidence=HXZ variant. Review the generated entry before using it as a final public corpus item.
