...
entry_type: strategy
id: openap_ebm_q
canonical_name: Enterprise Component of BM (Quarterly)
aliases:
- EBM_q
- Enterprise component of BM
one_line: Cross-sectional equity anomaly that uses Enterprise component of BM to rank
...
## Signal intuition
The signal definition is: (ceq + che - dltt - dlc - dc - dvpa+ tstkp ) / (mve\_c + che - dltt - dlc - dc - dvpa+ tstkp). Exclude if price less than 5. The source direction is to rank stocks by the signal and form the source-defined long-short spread.

## Construction sketch
```text
for each rebalance date:
    compute EBM_q for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=EBM_q; category=valuation; data=Accounting; evidence=HXZ variant. Review the generated entry before using it as a final public corpus item.
