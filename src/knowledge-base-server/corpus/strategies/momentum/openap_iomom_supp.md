---
entry_type: strategy
id: openap_iomom_supp
canonical_name: Suppliers momentum
aliases:
- Suppliers momentum
- iomom_supp
one_line: Cross-sectional equity anomaly that uses Suppliers momentum to long high-signal
...
---
## Thesis
Suppliers momentum is represented in the OpenAP signal catalog as a lead lag predictor. The corpus entry is a source-derived reference for strategy discovery, not a claim that the current backtest engine can reproduce the OpenAP portfolio exactly.
...
for each rebalance date:
    compute iomom_supp for every eligible stock using OpenAP data rules
...
## Notes
Source row: acronym=iomom_supp; category=lead lag; data=Other; evidence=t=5 in mv reg. Review the generated entry before using it as a final public corpus item.
