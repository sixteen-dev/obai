...
_LIST_KEYS = ("results", "markets", "trades", "holders", "history", "activity")


def truncate_response(data: dict[str, Any]) -> dict[str, Any]:
    """Truncate response to fit within MCP response size limits.

    Trims any list-valued keys from the tail until the serialized
    payload fits under max_response_chars.
