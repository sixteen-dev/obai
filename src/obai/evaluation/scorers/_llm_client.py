...
def get_judge_model() -> str:
    """Return the configured eval judge model ID."""
    return get_config().eval_judge_model


# Keep as importable constant for default parameter values.
# Callers needing the live config value should use get_judge_model().
DEFAULT_JUDGE_MODEL = "claude-sonnet-4-5"
_MAX_TOKENS = 4096
_REQUEST_TIMEOUT = 60.0  # seconds; prevents stalled upstream from blocking scoring
...
async def structured_completion(
    *,
    ...
    response = await client.chat.completions.create(**create_kwargs, timeout=_REQUEST_TIMEOUT)
    if not response.choices:
        msg = f"Empty choices in API response for model {model}"
        raise ValueError(msg)
